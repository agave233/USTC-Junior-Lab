	...
	for(int64_t i = 0;i < n;i++)
	{
		...
		for(int64_t j = i + 1;j < n;j++)
		{
			...
			else
			{
				...
				if(p <= exp((double)(count - curr_count) / T))
				{
					count = curr_count;
					update_diag(i, j);
					exchange(i, j);			
...
int main(int argc, char const *argv[])
{
	...
	// cout << "[INFO] simulated_annealing for n queens gets started." << endl;
	...
	{
		// cout << "[ERROR] wrong input of n(1~1000000) or m > " << n * (n - 1) / 2 <<endl;
		cout << -2 << endl;
		return -1;
	...
	// cout << "[INFO] executing simulated_annealing algorithm..." << endl;
	start = clock();
	int k = 0;
