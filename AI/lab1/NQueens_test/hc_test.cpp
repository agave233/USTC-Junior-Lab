...
// 极其容易陷入局部最优？优化2
// m取得很大时，1.会很慢 2.核心已转储，怎么破？
void random_init_state(int k)
{
	count = 0;
	for(int i = 0;i < n;i++)
		queen_state[i] = i;
	...
	for(int64_t i = 0;i < n;i++)
	{
		...
		for(int64_t j = i + 1;j < n;j++)
		{
			...
			if(curr_count < min_count && curr_count >= m)
			{
				...
				count = min_count;
				// cout << count << endl;
				// cout << i << " " << j << " " << curr_count << endl;
				// cout << diag1_nums[i + queen_state[i]] << ' ' << diag2_nums[i - queen_state[i] + n - 1] << ' ' <<\
				// 	 	diag1_nums[j + queen_state[j]] << ' ' << diag2_nums[j - queen_state[j] + n - 1] << endl;
				// cout << diag1_nums[i + queen_state[j]] << ' ' << diag2_nums[i - queen_state[j] + n - 1] << ' ' <<\
				// 	 	 diag1_nums[j + queen_state[i]] << ' ' << diag2_nums[j - queen_state[i] + n - 1] << endl;
			}
		}
	}
...
int main(int argc, char const *argv[])
{
	...
	// fout << setprecision(10) << totaltime << "s" << endl;
	// if(n < 16)
	// 	show_queen();
	count = 0;
	calcu_count();
	cout << count << " " << totaltime;
